...
(function ($, _, Backbone, Backgrid, lunr) {
  ...
  */
  var ServerSideFilter = Backgrid.Extension.ServerSideFilter = Backbone.View.extend({
    ...
    /* I modified this to be bootstrappy (it needs to take up the
     * entire width of the div), and to include a refresh button. I
     * wanted to put the refresh button on the same row as this
     * filter, but placing it in a separate div caused spacing issues
     * when the page was resized a lot. Putting it in this div is
     * pretty sloppy, since it's not actually part of the filter,
     * but I couldn't cleanly put it in an HTML template, so ... */
    template: _.template('<div class="input-prepend input-append input-group"><span class="add-on input-group-addon"><i class="icon-search"></i></span><input class="form-control" type="text" <% if (placeholder) { %> placeholder="<%- placeholder %>" <% } %> name="<%- name %>" /><span class="add-on input-group-addon"><a class="close" href="#">&times;</a></span><span class="add-on input-group-addon" id="refreshSpan"><div type="button" id="refreshButton" class="btn btn-xs btn-primary"><i class="icon-refresh"></i> Refresh </div></span></div>'),
